namespace player2_sdk
{
    ...
    public class Player2NpcResponseListener : MonoBehaviour
    {
        ...
        public void StartListening()
        {
            ...
            {
                Debug.Log("Cannot start listening: Game ID is not set");
                return;
            }
        ...
        private async Awaitable ListenForResponsesAsync()
        {
            while (_isListening && this != null)
            {
                ...
                {
                    Debug.Log($"Error in response listener: {ex.Message}");
        ...
        private void ProcessLine(string line)
        {
            ...
            {
                Debug.Log($"Failed to parse response line '{line}': {e.Message}");
            }
        }
        ...
        private async Awaitable HandleReconnectionAsync()
        {
            ...
            {
                Debug.Log($"Max reconnection attempts ({_maxReconnectAttempts}) reached. Stopping listener.");
                _isListening = false;
                return;
            }
